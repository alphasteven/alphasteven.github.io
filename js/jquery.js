...
(function($) {
    (function(a, b) {
        ...
        function n(b) {
            ...
            d.length ? (d.join() === "0,0,0,0" && a.extend(d, k.transparent),
            c) : k[b]
        }
        function o(a, b, c) {
            return c = (c + 1) % 1,
        ...
        g.hsla.to = function(a) {
            ...
            var b = a[0] / 255, c = a[1] / 255, d = a[2] / 255, e = a[3], f = Math.max(b, c, d), g = Math.min(b, c, d), h = f - g, i = f + g, j = i * 0.5, k, l;
            return g === f ? k = 0 : b === f ? k = 60 * (c - d) / h + 360 : c === f ? k = 60 * (d - b) / h + 120 : k = 60 * (b - c) / h + 240,
            j === 0 || j === 1 ? l = j : j <= 0.5 ? l = h / i : l = h / (2 - i),
        ...
        g.hsla.from = function(a) {
            ...
            var b = a[0] / 360, c = a[1], d = a[2], e = a[3], f = d <= 0.5 ? d * (1 + c) : d + c - d * c, g = 2 * d - f, h, i, j;
        ...
        l(g, function(c, e) {
            ...
            l(g, function(b, e) {
                ...
                f.fn[b] = function(f) {
                    ...
                    g = a.type(f)),
                    f == null && e.empty ? this : (g === "string" && (k = d.exec(f),
    ...
    jQuery.fn.framerate = function(a) {
        var b = jQuery.extend({
            framerate: 30,
            logframes: false
        }, a);
        var c = Math.floor(1000 / b.framerate);
        jQuery.extend(jQuery.fx.prototype, {
            custom: function(i, h, g) {
                ...
                if (f() && jQuery.timers.push(f) && !jQuery.timerId) {
                    jQuery.timerId = setInterval(jQuery.fx.tick, c)
                }
            }
        });
        ...
        jQuery.extend(jQuery.fx, {
            tick: function() {
                if (b.logframes) {
                    ...
                    console.log(Math.floor(1000 / (e - d)));
                    d = e
                }
                var g = jQuery.timers;
...
$.ShowMessage = function(settings, callback) {
    settings = $.extend({
        ...
        backgroundchangeat: 3000,
        backgroundspeedchange: 2000,
        timeout: undefined,
    }, settings);
    if (settings.texts == undefined) {
        alert("Need at least 1 text to display.");
        return false
    }
    var content = "";
    content += '<div id="msBackground">';
    content += '<span id="msSmallText"></span>';
    content += '<span id="msBigText"></span>';
    content += "</div>";
    ...
    if (settings.timeout != undefined) {
        ...
        if (settings.texts != undefined) {
            var SmallText = $("#msSmallText");
            var BigText = $("#msBigText");
            SmallText.html(settings.texts[0].text);
            BigText.html(settings.texts[0].text);
            randomTextPosition();
            SmallText.animate({
                opacity: 1
            }, {
            ...
            SmallText.animate({
                ...
                duration: settings.textchangeat,
                queue: false
            }, "linear");
            BigText.animate({
                opacity: 0.4
            ...
            clearInterval(TextTimer);
            SmallText.attr("currentindex", 0);
            var LeftRight = 0;
            var SmallLeft = "";
            var BigLeft = "";
            TextTimer = setInterval(function() {
                BigText.fadeOut(500);
                SmallText.fadeOut(500, function() {
                    BigText.clearQueue();
                    SmallText.clearQueue();
                    randomTextPosition();
                    SmallText.fadeIn(800);
                    BigText.fadeIn(800);
                    var TextIndex = SmallText.attr("currentindex");
                    TextIndex = (TextIndex * 1) + 1;
                    if (TextIndex > settings.texts.length - 1) {
                        TextIndex = 0
                    }
                    SmallText.attr("currentindex", TextIndex);
                    SmallText.html(settings.texts[TextIndex].text);
                    BigText.html(settings.texts[TextIndex].text);
                    if (LeftRight == 1) {
                        LeftRight = 0;
                        SmallLeft = "+=100";
                        BigLeft = "-=100"
            ...
            }, settings.textchangeat - 1000)
        }
        clearInterval(BackTimer);
        if (settings.backgrounds == undefined) {
            $("#msBackground").animate({
                backgroundColor: "#662d91",
            })
        } else {
            $("#msBackground").animate({
                backgroundColor: settings.backgrounds[0].color,
            });
            var BackgroundDiv = $("#msBackground");
            BackgroundDiv.attr("colorindex", 0);
            if (settings.backgrounds.length > 1) {
                BackTimer = setInterval(function() {
                    var NextColor = BackgroundDiv.attr("colorindex");
                    NextColor = (NextColor * 1) + 1;
                    if (NextColor > settings.backgrounds.length - 1) {
                        NextColor = 0
                    }
                    BackgroundDiv.attr("colorindex", NextColor);
                    BackgroundDiv.animate({
                        backgroundColor: settings.backgrounds[NextColor].color,
                    }, settings.backgroundspeedchange)
                }, settings.backgroundchangeat)
            }
        }
    };
    ...
    function randomTextPosition() {
        ...
        var NewTop = randomFromInterval(50, (WindowHeight - 30));
        var SmallText = $("#msSmallText");
        var BigText = $("#msBigText");
        var TextSpacing = 50;
        if (WindowWidth < 500) {
            SmallText.css("font-size", "20px");
            BigText.css("font-size", "30px");
            TextSpacing = 30
        } else {
            SmallText.css("font-size", "30px");
            BigText.css("font-size", "60px")
        }
        var xPos = "";
        var SmallWidth = SmallText.width();
        var MaxLeft = WindowWidth - SmallWidth - 70;
        var RandomLeft = randomFromInterval(30, MaxLeft);
        var SmallX = "";
        var Bigx = "";
        if (xMovin == 0) {
            SmallX = RandomLeft - 50;
            Bigx = RandomLeft + 50;
            xMovin = 1
        } else {
            SmallX = RandomLeft + 50;
